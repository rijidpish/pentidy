...
report_custom() # Download .odt report template from GitHub and update / add relevant fields
{
	...
	if [ -z $enable_report ]; then
		...
		wget https://github.com/rijidpish/scripts/blob/master/report-file.odt?raw=true -O $custom_path$client_name/report/report-$client_name.odt &> /dev/null && echo " $txtgrn$txtchk$txtgry  Got: file://$custom_path$client_name/report/report-$client_name.odt" || echo "$txtred$txtcro$txtgry  Unable to download template!"$txtrst
